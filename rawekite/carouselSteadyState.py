...
#         print "Dot bounds: \n", self.dotBounds
    ...
    def getSteadyState( self ):
        ...
        guessVec = C.DMatrix([self.guess[n] for n in self.dvsNames] + [self.dotGuess[n] for n in self.dae.xNames()])          
        boundsVec = [self.bounds[n] for n in self.dvsNames] + [self.dotBounds[n] for n in self.dae.xNames()]
        
        self.solver.setInput(self.lbg, 'lbg')
        self.solver.setInput(self.ubg, 'ubg')
        self.solver.setInput(guessVec, 'x0')
        lb, ub = zip(*boundsVec)
        self.solver.setInput(C.DMatrix(lb), 'lbx')
        self.solver.setInput(C.DMatrix(ub), 'ubx')
    
        self.solver.solve()
        ret = self.solver.getStat('return_status')
        assert ret in ['Solve_Succeeded', 'Solved_To_Acceptable_Level'], 'Solver failed: ' + ret
        
        xOpt = self.solver.output('x')
        k = 0
        sol = {}
        for name in self.dvsNames:
        ...
        for name in self.dae.xNames():
            if abs( dotSol[ name ] ) > 1e-8 and name not in ["sin_delta", "cos_delta"]:
                print "Warning: dotSol[ %s ] = %5.3f" % (name, dotSol[ name ])
